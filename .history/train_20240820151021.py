...
def train(epoch):
    ...
    for data in train_loader:
        ...
        optimizer.step()
    epoch_train_ROC = evaluator.eval({'y_pred': torch.cat(list_pred),
                                     'y_true': torch.cat(list_labels)})['rocauc']
    return epoch_train_ROC, total_loss / len(train_loader.dataset)
...
for epoch in pbar:
    ...
    train_roc, train_loss = train(epoch)
    val_roc, val_loss = test(val_loader)
    scheduler.step(val_roc)
    logger.info(
        f'Epoch: {epoch:02d}, train_Loss: {train_loss:.4f}, train_roc:{train_roc:.4f}, Val_loss:{val_loss:.4f},Val_roc: {val_roc:.4f}')
    writer.add_scalars('Loss', {'train': train_loss, 'val': val_loss}, epoch)
    writer.add_scalars('ROC', {'train': train_roc, 'val': val_roc}, epoch)
    if val_roc > best:
        best = val_roc
